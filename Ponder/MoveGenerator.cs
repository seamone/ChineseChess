...
namespace Ponder.ChineseChess
{
    ...
    public partial class MoveGenerator
    {
            ...
            foreach (int to in MoveGenerator.PRESET_RED_PAWN[from])
            {
                ...
                if (capture == Board.NONE)
                {
                    if (Board.Rank(from) == Board.Rank(to)) 
                        ...
                        AddRedMoveIrreversible(from, to, Board.RED_PAWN, movelist, ref countMove);
                }
            ...
            foreach (int to in MoveGenerator.PRESET_BLACK_PAWN[from])
            {
                ...
                if (capture == Board.NONE)
                {
                    if(Board.Rank(from) == Board.Rank(to)) 
                        ...
                        AddBlackMoveIrreversible(from, to, Board.BLACK_PAWN, movelist, ref countMove);
                }
        ...
        private void AddRedMoveIrreversible(int from, int to, int piece, Move[] movelist, ref int countMove)
        {
            Move m = new Move(from, to, piece, 0);
            if (IsLegalRedMove(m))
            {
        ...
        private void AddBlackMoveIrreversible(int from, int to, int piece, Move[] list, ref int countMove)
        {
            Move move = new Move(from, to, piece, 0);
            if (IsLegalBlackMove(move))
            {
                move.Irreversible = true;
                list[countMove++] = move;
            }
        }
        ...
        private void AddBlackCaptureMove(int from, int to, int piece, int capture, Move[] list, ref int countMove)
        {
            ...
            if (IsLegalBlackMove(move))
            {
                move.Irreversible = true;  // 不可逆的着法，用于重复局面检测
                list[countMove++] = move;
            }
        }
        ...
        public static int GenAllMoveList(Board board, Move[] movelist)
        {
            ...
            //把吃子的着法排在前面
            //Sort(movelist, countMove);
            return countMove;
        }

        private static void Sort(Move[] movelist, int countMove)
        {
            int start = 0;
            int end = countMove-1;
            ...
            while(start < end)
            {
                while (start < end && movelist[start].Capture == 0) ++start;
                while (start < end && movelist[end].Capture != 0) --end;
                if (start < end)
                {
                    //swap
                    Move temp = movelist[start];
                    movelist[start] = movelist[end];
                    movelist[end] = temp;
                    ++start;
                    --end;
                }
        ...
        /// <summary>
        /// 用一个递归方法，来测试着法生成是否正确，也可以用来测试着法生成的性能
        /// </summary>
        /// <param name="depth">递归的深度，红方走一步或黑方走一步都算1层</param>
        /// <returns>产生着法的总数</returns>
